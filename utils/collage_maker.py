...
def make_collage(images, filename, width, init_height, count = 3):
    """
    ...
    width = 1024
    out_height = 1024
    print("begin............%i %i" % (width, int(out_height)))
    ...
    for coef, imgs_line in coefs_lines:
        if imgs_line:
            ...
            for img_path in imgs_line:
                ...
                k = 1;#(init_height / 1) / img.size[1]
                # if k > 1:
                #     img = img.resize((int(img.size[0] * 1), int(img.size[1] * 1)), Image.ANTIALIAS)
                # else:
                #     img.thumbnail((int(width / coef), int(init_height / 1)), Image.ANTIALIAS)

                # print("%i %i" % (int(x), int(y)))
                size = int(1024.0/count)
                if collage_image:
                    collage_image.paste(img, ((nn % count) * size, int(nn / count) * size))
                # x += img.size[0] + margin_size
                nn = nn + 1
            y += int(init_height / 1) + margin_size
